...
std::vector<Position> Pawn::getValidMoves(Position currentPos, const Board& board) const {
    ...
    if (board.isinBounds(forward) && board.getTile(forward).isWalkable() && !board.getTile(forward).hasPiece()) {
    ...
    if (!this->hasMoved()) {
        ...
        if (board.isinBounds(doubleForward) && 
            board.getTile(forward).isWalkable() && !board.getTile(forward).hasPiece() && 
            board.getTile(doubleForward).isWalkable() && !board.getTile(doubleForward).hasPiece()) {
    ...
    if (enPassantTarget.x != -1 && enPassantTarget.y != -1) {
        if (board.getTile(enPassantTarget).isWalkable() &&
            (currentPos.x - 1 == enPassantTarget.x || currentPos.x + 1 == enPassantTarget.x) &&
